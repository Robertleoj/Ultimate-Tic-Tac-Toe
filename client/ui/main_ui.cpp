...
bool MainUI::init_menu_buttons(){
    ...
    this->main_menu_button_map = std::unordered_map<Button *, MainMenuButtons>{
        ...
        {main_menu_buttons[1], MM_ONLINE},
        {main_menu_buttons[2], MM_PLAY_AI},
        {main_menu_buttons[3], MM_EXIT}
    };
...
bool MainUI::handle_main_menu_click(){
    ...
    switch(clicked){
        case MM_TWO_PLAYER:
            return this->gameui->run(PM_LOCAL_TP);
        case MM_ONLINE:
            return this->gameui->run(PM_ONLINE);
        case MM_PLAY_AI:
...
bool MainUI::handle_choose_diff_click(){
    ...
    return this->gameui->run(PM_PLAY_AI, diff);
}
